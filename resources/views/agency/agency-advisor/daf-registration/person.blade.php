...
$donorAccountInfo = !empty($model->email ?? null);
...
$selectedPhoneType = \App\Models\PhoneType::getPrimaryPhoneTypeOrAny();
@endphp
        ...
        <div class="col-md-3 mb-1">
            <select name="phone_type" class="form-control" required>
                @foreach ($phoneTypes as $val => $label)
                    <option value="{{ $val }}" {{ old('phone_type', $model->phone_type ?? $selectedPhoneType) == $val ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </div>
    @else
        <input type="hidden" name="phone_type" value="{{ old('phone_type', $model->phone_type ?? $selectedPhoneType) }}">
    @endif
</div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="email" name="email" id="email" class="form-control"
                   value="{{ old('email', $model->email ?? '') }}" readonly>
        </div>
    </div>
@else
    <div class="form-group row">
