...
<form method="POST" action="{{ route('post-agency-daf-successors-individuals', $id) }}"
      id="daf-successors-individuals-{{ $individual->key }}">
    @csrf
...
@include('agency.agency-advisor.daf-registration.person', ['model' => $individual])

<div class="form-group row">
    <label for="id_share_value" class="col-md-3 col-form-label form-multi-line-label text-right pr-0">% of Giving Account</label>
    <div class="col-md-3">
        <input type="number" name="share_value" id="id_share_value" class="form-control"
               onchange="handleChange(this);"
               value="{{ old('share_value', $individual->share_value ?? '') }}" required>
    </div>
</div>

@include('agency.agency-advisor.daf-registration.address', ['model' => $individual])
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        ...
                        window.location.href = "{{route("delete-agency-successors-individual", $id)}}?contact_id="+id;
                    }
                }
            }
        });
