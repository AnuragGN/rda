...
$contributionStatus = $dafInfo['status'][\App\Models\DAFAccount::DAF_CONTRIBUTIONS] ?? null;
$showSkipBtn = $contributionStatus === \App\Models\DAFAccount::LINK_SAVED;
@endphp
...
@section ('content')
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="form-group">
                        <p class="form-title">Securities or Mutual Funds
                            @if($showSkipBtn)
                        ...
                        @foreach($securities as $i => $security)
                            ...
                            @if($security->isNew && count($securities) > 1)
                                ...
                                <div id="id_add_security" style="display: none" class="daf-form-card" >
                                    <form method="POST" action="{{ route('post-agency-daf-contributions-securities', $id) }}"
                                          class="daf-contribution-form" id="daf-contribution-{{ $security->key }}">
                                        @csrf
                                        <input type="hidden" name="key" id="id_security_key" value="{{ $security->key }}">
                                        <input type="hidden" name="isNew" value="{{ $security->isNew }}">

                                        @include("agency.agency-advisor.daf-registration._form_security")
                                    </form>
                                </div>
                            @else
                                    ...
                                    <form method="POST" action="{{ route('post-agency-daf-contributions-securities', $id) }}"
                                        ...
                                        @if(!$security->isNew)
                                            <div class="offset-md-9 col-md-3 text-right mb-1">
                                                <a href="javascript:void(0);" class="delete-organization" id="id_delete_orgs-{{$security->key}}" onclick="deleteSecurity(this)" data="{{$security->key}}" style="">Delete</a>
                                            </div>
                                        @endif
                                    </form>
                                </div>
                ...
                <div class="col-md-4">
                    @include(\App\Models\ClientInfo::clientViewFor("daf-registration.side-pane-securities", "agency.agency-advisor."))
                </div>
            </div>
    ...
    <script>
        ...
        $(function(){
            ...
            if (securities) {
                $('#id_contributions_tab a[href="#securities"]').tab('show'); // Select tab by name
            }
        });
                ...
                buttons: {
                    ...
                    yes: {
                        ...
                        action: function () {
                            ...
                            window.location.href = "{{route("delete-agency-contributions-security", $id)}}?key="+key;
