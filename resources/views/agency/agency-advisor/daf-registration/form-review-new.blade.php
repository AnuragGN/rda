...
$stepEnabled = function($key) use ($menu) {
    foreach ($menu as $parent) {
        if ($parent['key'] === $key) return true;
        if (!empty($parent['children'])) {
            foreach ($parent['children'] as $child) {
                if ($parent['key'] . '.' . $child['key'] === $key) return true;
            }
        }
    }
    return false;
...
<div class="col-md-12">
    @if ($stepEnabled('successors') && $successors && $successors->endowment->isSelected == false)
...
<div class="col-md-12">
    ...
    @if ($stepEnabled('additional_donor') && !empty($addDonorInfo['donors']))
    ...
    @if ($stepEnabled('successors') && $successors)

        @if (optional($successors->endowment)->isSelected)
    ...
    @endif
    @if (isset($contributions->cash) && (isset($contributions->cash->wire_pay) || isset($contributions->cash->check_pay)))
        <div class="form-group">
    ...
    @if (!empty($contributions->securities))
    ...
    @if (!empty($contributions->others->is_active))
