...
if($secAssignee){
    ...
    $work_status = $secAssignee->status;
}
...
<tr>
    ...
    <td style="text-align: left !important;">
       ...
       @if($work_status != '')
            <br><small><b>Work Status :</b> {{ config('dropdown.support_staff_status')[$work_status] }} </small>
        @else
        @endif 
    </td>
    <td>
        <input type="hidden" class="ticket-class" value="{{ $tickets->status }}">
        <small><a title="View Ticket" href="/m/agency/ticket/view/{{ $tickets->id }}">
            <i class="fa fa-eye" aria-hidden="true" style="color:#00758f;"></i>
        </a></small> | 

        <small><a title="Edit Ticket" href="/m/agency/ticket/edit/{{ $tickets->id }}">
