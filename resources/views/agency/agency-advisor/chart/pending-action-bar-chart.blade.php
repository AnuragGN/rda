...
?>
@if (count($pendingActionChartData))

    <div class="bar-chart-container chart">
        <canvas id="barChartPendingAction" width="300" height="200"></canvas>
    </div>

    <script>
        var pendingActionChartData = <?php echo json_encode($pendingActionChartData)?>;
        var theBgColor = '<?php echo $color?>';
        var borderWidth = '<?php echo $borderWidth?>';

        document.addEventListener("DOMContentLoaded", function() {
            var ctx = document.getElementById('barChartPendingAction').getContext('2d');
            window.myBar = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: pendingActionChartData.label,
                    datasets: [{
                        label: 'Pending Actions',
                        data: pendingActionChartData.data,
