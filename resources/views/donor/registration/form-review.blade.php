...
{{--{!! Form::model($donor, ['method' => 'POST', 'files' => false, 'route' => ['post-daf-account-info'], 'id' => 'daf-account-info-form']) !!}--}}
<div class="col-md-12">
    ...
    @include('donor.registration.review-person-info', ['person' => $donorInfo['donor'], 'personFields' => $personFieldsDonor])
    @include('donor.registration.review-address-info', ['address' => $donorInfo['donor']])

    @if (in_array(App\Helpers\Data::DAFR_DONOR_INFO_MAILING_ADDRESS, $personFieldsDonor))
        @include('donor.registration.review-mailing-address-info', ['mAddress' => $donorInfo['donor']])
    @endif
    ...
    @if( isset($additionalDonors->donors) )
        ...
        @foreach ($addDonorInfo['donors'] as $donor)
            ...
            @include('donor.registration.review-person-info',['person' => $donor, 'personFields' => $personFieldsAdditionalDonor])

            @include('donor.registration.review-address-info', ['address' => $donor])

            @if (in_array(App\Helpers\Data::DAFR_DONOR_INFO_MAILING_ADDRESS, $personFieldsAdditionalDonor))
                @include('donor.registration.review-mailing-address-info', ['mAddress' => $donor])
            @endif
        ...
        @if( isset($successors->individuals) && count($successors->individuals))
            ...
            @foreach($individuals['individuals'] as $individual)
                ...
                @if( isset($individual->isNew) == false)
                    ...
                    @include('donor.registration.review-person-info',['person' => $individual, 'personFields' => $personFieldsSuccessorsIndividual])
                    @include('donor.registration.review-address-info', ['address' => $individual])
                @endif
            ...
            @foreach($orgs['organizations'] as $index => $organization)
                ...
                @if( isset($organization->isNew) == false)
                    ...
                    <div class="form-group row mb-0">
                        ...
                        @include('donor.registration.review-address-info', ['address' => $organization])
