...
<main>
    <div class="title">
        {{ \App\Helpers\GnUtils::isDonorSession() ? 'Grant' : 'Disbursement' }} History
    </div>

    @if (isset($params['organization_id']))
        <?php $organization = \App\Models\Organization::find($params['organization_id']); ?>
        <div class="filter">{{ $organization ? $organization->name : '' }}</div>
    @endif
    ...
    <table>
        ...
        @foreach ($grants as $key => $grant)
            <tr>
                <td>{{ \App\Helpers\GnUtils::customDate($grant->grant_date) }}</td>
                <td>{{$grant->grantee}}</td>
                <td>{{ \App\Helpers\GnUtils::money($grant->amount) }}</td>
            </tr>
        @endforeach
    </table>
