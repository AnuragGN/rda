...
@if($phone->contact_phone_id and $phone->is_primary != 'Y')
    <a href="javascript:void(0);" id="id_delete_phone" style="">Delete Phone</a>

    <form name="deleteContactPhoneForm" id="deleteContactPhoneForm" method="POST"
          action="{{ route('profile-phone-delete') }}">
        @csrf
        <input type="hidden" name="id" value="{{$phone->getModelId()}}" />
    </form>
                ...
                buttons: {
                    yes: {
                        ...
                        action: function(){
                            ...
                            document.deleteContactPhoneForm.submit();
                        }
                    },
                    no: {
                        text: 'Cancel',
